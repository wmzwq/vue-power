<template>
  <div class>
    <van-nav-bar title="视频监测" left-arrow @click-left="onClickLeft" />
    <div class="flex">
      <van-dropdown-menu>
    ...
    <div class="flex">
      <div class="statistics">
        <div class="block fl" @click="open(item.DeviceToken,item.Evideoid)" :key="index" v-for="(item, index) in list" >
          <div class="statistics_p">
            <img src="../../assets/img/拓扑.png" alt />
          </div>
          <p class="statistics_t flex">{{item.DeviceName}}</p>
        </div>
    </div>
  </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    open (token, id) {
      var url = 'https://open.ys7.com/ezopen/h5/iframe_voice?url=ezopen://open.ys7.com/' + id + '/1.live&autoplay=1&accessToken=' + token
      url = 'http://122.228.89.215:8888/CordovaAppHistory/index.html?' + url
      console.log(url)
